 ...
 * @since 2021/7/16
 */
public class CustomObjectConvert {
    public CustomObjectConvert() {
    }
    ...
    public static String digitUppercase(double n) {
        ...
        for (integerPart = 0; integerPart < fraction.length; ++integerPart) {
            s = s + (digit[(int) (Math.floor(n * 10.0D * Math.pow(10.0D, integerPart)) % 10.0D)]
                    + fraction[integerPart]).replaceAll("(零.)+", "");
        }
    ...
    public static String utf8Str(Object obj) {
        return str(obj, CommonConstants.DEFAULT_CHARSET);
    }
}
